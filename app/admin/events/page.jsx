...
export default async function AdminEvents({ searchParams }) {
  ...
  const rows = isAdmin
    ...
    : (f
        ? await sql`SELECT * FROM events WHERE submitted_by = ${user.id} AND status = ${f} ORDER BY start_at DESC`
        : await sql`SELECT * FROM events WHERE submitted_by = ${user.id} ORDER BY start_at DESC`);

  return (
    <>
      <div className="hero" style={{ padding: '24px 0 0' }}><h1>{isAdmin ? 'EVENTS' : 'MY EVENTS'}</h1></div>
      <div className="cta-row" style={{ marginTop: 14 }}><Link className="btn" href="/admin/events/new">{isAdmin ? '+ CREATE EVENT' : '+ SUBMIT EVENT'}</Link></div>
        ...
        {rows.map((e) => (
          <tr key={e.id}>
            ...
            <td style={{ whiteSpace: 'nowrap' }}>
              <Link className="btn btn-ghost btn-sm" href={`/admin/events/${e.id}/edit`}>{isAdmin ? 'EDIT' : 'VIEW'}</Link>{' '}
              {e.status === 'published' && <Link className="btn btn-ghost btn-sm" href={`/events/${e.slug}`} target="_blank">VIEW</Link>}
            </td>
          </tr>
        ))}
      </tbody></table></div>
      {rows.length === 0 && <p className="muted" style={{ marginTop: 16 }}>{isAdmin ? 'No events here yet.' : "You haven't submitted any events yet."}</p>}
    </>
  );
}
